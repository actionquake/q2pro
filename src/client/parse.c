...
static void CL_ParseConfigstring(int index)
{
    ...
    if (index < 0 || index >= cl.csr.end) {
        Com_Error(ERR_DROP, "%s: bad index: %d, end_index: %d, max_len: %d", 
        __func__, index, cl.csr.end, MAX_CONFIGSTRINGS);
    }
...
static void CL_ParseServerData(void)
{
    ...
    if (cls.serverProtocol != protocol) {
        ...
        if (protocol == PROTOCOL_VERSION_EXTENDED || protocol == PROTOCOL_VERSION_AQTION_EXTENDED_LIMITS) {
            cl.csr = cs_remap_new;
    ...
    if (cls.serverProtocol == PROTOCOL_VERSION_R1Q2) {
        ...
        if (cls.protocolVersion >= PROTOCOL_VERSION_Q2PRO_EXTENDED_LIMITS || cls.protocolVersion >= PROTOCOL_VERSION_AQTION_EXTENDED_LIMITS) {
            i = MSG_ReadWord();
...
static void CL_ParseMuzzleFlashPacket(int mask)
{
    ...
    if (entity < 1 || entity >= cl.csr.max_edicts)
        Com_Error(ERR_DROP, "%s: bad entity: %d, max edicts: %d", __func__, entity, cl.csr.max_edicts);
